...
async def sql_add_train(train_number, time_departure):
    cur.execute('INSERT INTO data VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
                (time_departure, train_number, 0, 0, 0, 0, 0, 0, 0))
    base.commit()
...
async def sql_update_train(train_number, time_departure,  all_seats):
    cur.execute('UPDATE  data SET sitting = ?,'
                ' platzkarte_low = ?, platzkarte_up = ?,'
                ' compartment_low = ?, compartment_up = ?,'
                ' suite = ?, soft = ?'
                ' WHERE train == ? AND time_departure = ?',
                (all_seats[0], all_seats[1], all_seats[2], all_seats[3],
                            all_seats[4], all_seats[5], all_seats[6],  train_number, time_departure))
    base.commit()
...
async def sql_delete_old():
    ...
    for i_table in table:
        if datetime.strptime(str(i_table[0]), "%Y-%m-%d %H:%M:%S") < datetime.now():
